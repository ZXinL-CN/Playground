...
import boardImgUrl from "@/assets/images/board.jpg";
//落子数组
var chessBoard: any = [];
//默认玩家黑棋先手，电脑白棋后手
var me = true;
...
onMounted(() => {
  ...
  cnt = cnv.getContext("2d") as CanvasRenderingContext2D;
  cnt.strokeStyle = "#000";
...
//绘制棋子
function oneStep(i: any, j: any, me: any) {
  //i,j分别是在棋盘中的定位，me代表黑棋还是白棋
  cnt.beginPath();
  cnt.arc(15 + i * 30, 15 + j * 30, 13, 0, (Math.PI / 180) * 360, false); //半径为13
  cnt.closePath();
  var gradient = cnt.createRadialGradient(
    15 + i * 30,
    15 + j * 30,
    0,
    15 + i * 30 + 2,
...
//计算机落子
function computerAI() {
  var myScore: any = []; //定义两个二维数组，用于存放每个点的分值
  var computerScore: any = [];
  var max = 0; //落子的价值
  var u = 0; //落子的坐标
  var v = 0;
...
<template>
  <div class="Gobang">
    <h1>hello</h1>
    <img id="myimg" :src="boardImgUrl" alt="" />
